...
import '15AddingRecinpe.dart';
import '16RecipeInformation.dart';
...
class _FoodPageState extends State<FoodPage> {
  ...
  void initState() {
    for (int i = 0; i < ingredientList.length; i++){
      if(widget.name == ingredientList[i].name){
        Cal = ingredientList[i].cal;
        Fats = ingredientList[i].fats;
        Protein = ingredientList[i].protein;
        Carbs = ingredientList[i].carbs;
        index = i;
      }
    }
            ...
            Navigator.of(context).push(
                MaterialPageRoute(
                    builder: (context) => AddingRecipe(
                      ...
                      protein: Protein,
                      carbs: Carbs,
                      nutritionList: ingredientList[index].nutrientList,
                    )
                )
            );
          });
